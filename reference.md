...
| TDR (P1: 141) during fullscreen gameplay, screen freezes then recovers | Pre-Pass-4: launcher's unconditional GPU bus reset + MPO compositing exceeded VFIO interrupt latency budget. Post-Pass-4: resolved by launcher fixes (gpu_bus_reset removal, SIGTERM deferral). MPO is NOT independently causal — tested with MPO re-enabled across multiple matches with zero WATCHDOG dumps (2026-04-20). Alt+tab twice can unfreeze before full TDR. HAGS adds further scheduling pressure. | Keep MPO **enabled** (required for Hardware: Independent Flip). Set `ForceDirectFlip` = 1 in `HKLM\SYSTEM\CurrentControlSet\Control\GraphicsDrivers`. Disable HAGS: `HwSchMode` = 1. Remove stale `OverlayTestMode` and `DisableMPO` keys if present. Windows must be activated (the "Activate Windows" watermark is a DWM overlay that forces Composed: Flip). |
...
| VM fails to start: `unable to map backing store for guest RAM: Cannot allocate memory` | Volt's embedded Postgres (`huge_pages = try` default) opportunistically consumes hugepages from the pool reserved for QEMU. Even a small number of stolen pages (43 used + 63 reserved) is enough to prevent QEMU from mapping the full 48 GiB. | overwatch stops Volt Postgres (`pg_ctl stop`) in `ensure_services_stopped` and restarts it in `ensure_services_running`. Alternative fixes: set `huge_pages = off` in Volt's `postgresql.conf`, or increase `nr_hugepages` in GRUB to add headroom. |
...
Run via guest-exec (from the Python helper scripts on the host):
...
Note: Store app stubs in `C:\Users\<username>\AppData\Local\Microsoft\WindowsApps\` cannot
...
- [ ] If no drops: check Battle.net logs at `C:\Users\<username>\AppData\Local\Battle.net\Logs\` for `BNPresence ERROR_INTERNAL` — presence subscription failure causes disconnects regardless of network quality
- [ ] If BNPresence errors found: close Battle.net, delete `C:\Users\<username>\AppData\Local\Battle.net\Cache`, relaunch (in-app "Clear Cache" was removed from newer client versions — delete manually)
