...
|---|---|---|
| PCI bus addresses (`03:00.0`) | Different kernel/BIOS enumeration | Check `lspci -nn`, update VM XML and overwatch script |
| i2c device numbers (`/dev/i2c-4` through `/dev/i2c-10`) | Different driver probe order | Check `ls /sys/bus/i2c/devices/`, update overwatch `fuser` lines |
